...
//!
//! The bridge is started lazily by [`crate::Host::session_start`] when a
//! session is launched for a provider that should see issue memory, and torn
//! down when [`IssueBridgeService`] is dropped or [`Self::stop`] is called.
//!
//! Submodules:
...
impl IssueBridgeService {
    ...
    pub fn new(issues: Arc<IssueService>, events: EventSink<HostEvent>) -> Self {
        Self {
            issues,
            events,
...
mod tests {
    ...
    fn no_op_events() -> EventSink<HostEvent> {
        Arc::new(|_| Ok(()))
    }
    ...
    async fn bridge_requires_token_and_serves_info() {
        ...
        let bridge = IssueBridgeService::new(issues, no_op_events());
    ...
    async fn bridge_issue_methods_use_issue_service() {
        ...
        let events: EventSink<HostEvent> =
            Arc::new(move |event| sender.send(event).map_err(|error| error.to_string()));
        let bridge = IssueBridgeService::new(Arc::clone(&issues), events);
        ...
        let epic = issues
            .create_epic(
                &project,
                sworm_protocol::issues::IssueEpicCreateInput {
        ...
        let HostEvent::IssuesChanged(folder) = receiver
            .recv_timeout(std::time::Duration::from_secs(1))
            .expect("issues-changed event")
        else {
            panic!("received non-issues host event");
        };
        assert_eq!(folder, project.to_string_lossy().into_owned());
    ...
    #[tokio::test]
    async fn mutation_succeeds_when_notification_fails() {
        let issues = Arc::new(IssueService::new());
        let events: EventSink<HostEvent> = Arc::new(|_| Err("receiver closed".to_string()));
        let bridge = IssueBridgeService::new(issues, events);
        let project = temp_project("notification-failure");
        let info = bridge.ensure_running(&project).unwrap();

        let epic = rpc(&info, "epic.create", json!({"title":"Persisted epic"})).await;
        assert_eq!(epic.get("id").and_then(Value::as_str), Some("EPIC-1"));
