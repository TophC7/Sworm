import { DND_MIME, type DragPayload, type SwormDragKind } from '$lib/features/dnd/payload'
import { createHoverStore } from '$lib/features/dnd/hover-state.svelte'
import { computeZone, type Zone } from '$lib/features/dnd/overlay'
import { dragObserver, frameAt, type DragFrame } from '$lib/features/dnd/observer.svelte'
...
import type { PaneSlot, PaneState, SplitDirection } from '$lib/features/workbench/model'
import {
  canSplitPane,
  moveTabToPane,
  setActiveTab,
...
async function dispatchPaneDrop(
  ...
  for (const item of payload.items) {
    ...
    if (item.kind === 'file') {
      ...
      await openTextFile(projectId, item.path, { paneSlot: targetSlot })
      setFocusedPane(targetSlot)
      handled = true
      continue
    }
    ...
    if (item.kind === 'os-files') {
      ...
      for (let i = 0; i < relatives.length; i++) {
        ...
        await openTextFile(projectId, rel, { paneSlot: targetSlot })
        setFocusedPane(targetSlot)
        handled = true
      }
    }
