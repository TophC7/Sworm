...
<ScrollAreaPrimitive.Root class={cn('relative overflow-hidden', className)} {...rest}>
	...
	{#if orientation !== 'horizontal'}
		<ScrollAreaPrimitive.Scrollbar
			orientation="vertical"
			class="flex touch-none select-none p-px transition-opacity w-1.5 hover:w-2"
		>
	...
	{#if orientation !== 'vertical'}
		<ScrollAreaPrimitive.Scrollbar
			...
			class="flex touch-none select-none p-px transition-opacity h-1.5 hover:h-2 flex-col"
		>
			<ScrollAreaPrimitive.Thumb class="relative flex-1 rounded-full bg-edge" />
		</ScrollAreaPrimitive.Scrollbar>
	{/if}
