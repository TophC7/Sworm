...
{#if hasNixFiles}
	<DropdownMenuRoot>
		<DropdownMenuTrigger
			class="flex items-center gap-1 text-[0.68rem] {statusColor()} hover:text-bright transition-colors cursor-pointer bg-transparent border-none p-0"
		>
			{#if evaluatingNow}
				<LoaderCircle size={10} class="animate-spin" />
			{:else}
				<span
					class="w-[10px] h-[10px] shrink-0 bg-current"
			...
			{#if detection}
				...
				{#if detection.selected}
					...
					{:else if detection.selected.status === 'evaluating' || evaluatingNow}
						<div class="px-3 py-1.5 text-[0.72rem] text-muted flex items-center gap-1.5">
							<LoaderCircle size={10} class="animate-spin" />
							Evaluating...
						</div>
					{/if}
