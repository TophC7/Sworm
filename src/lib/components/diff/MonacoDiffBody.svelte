<!--
  ...
  models (DiffModelStore), editor config (pool), or the file list (DiffStack).
-->

<script lang="ts">
  import DiffBinaryPlaceholder from '$lib/components/diff/DiffBinaryPlaceholder.svelte'
  import { getDiffEditorPool, type PoolRef } from '$lib/diff/editorPool.svelte'
  ...
  $effect(() => {
    ...
    if (entry.height != null) height = entry.height
    ...
    void (async () => {
      ...
      if (stickyReuse && entry.viewState) {
        try {
          ref.editor.restoreViewState(stripModelState(entry.viewState))
        } catch {
