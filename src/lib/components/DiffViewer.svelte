...
<div class="diff-viewer-root flex min-h-0 flex-1 flex-col overflow-hidden">
  <!-- Toolbar -->
  ...
  <!-- Diff content -->
  {#if binary}
    <div class="flex flex-1 items-center justify-center text-[0.8rem] text-subtle italic">
      Binary file — diff not available
    </div>
  {:else if !hasDiffContent}
    <div class="flex flex-1 items-center justify-center text-[0.8rem] text-subtle italic">No changes</div>
  {:else}
