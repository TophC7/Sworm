...
Findings MUST be a table. Prose-list findings are hard to scan and disallowed. Use this structure:

```md
## Findings

Table with | # | Severity | Location | Issue | Fix |
...
- One row per finding. Do not split a finding across rows.
- `Location` must be a code-formatted `path:line` (or `path:start-end`). Multiple locations for the same finding go comma-separated in one cell.
- `Issue` stays tight — one or two sentences. Push long rationale into a follow-up paragraph below the table only when a row genuinely cannot be compressed.
- `Fix` is imperative and specific. "Refactor this" is not a fix; "Extract `useTrackedAsyncLoad(key, load)`" is.
- If you have zero findings, write `No findings.` instead of an empty table.